...
int main(int argc, char **argv)
{
    ...
    boost::shared_ptr<sensor_msgs::CameraInfo const> msg_cameraInfo = ros::topic::waitForMessage<sensor_msgs::CameraInfo>("/mobile_manip/d435i/depth/camera_info", n);  // TODO: make topic name configureable
    ...
    // std::string source_frameid = "t265_odom_frame";   // TODO: make topic name configureable
    std::string source_frameid = "t265_odom_frame";
    std::string target_frameid = "d435i_depth_optical_frame";
    ...
    // register callbacks
    ros::Subscriber sub_pose = n.subscribe("/mobile_manip/t265/odom/sample", 1, poseCallback, ros::TransportHints().udp());  // min. latency: queue size = 1, use UDP
    // ros::Subscriber sub_depth = n.subscribe("d400/depth/image_rect_raw", 1, depthCallback, ros::TransportHints().udp());
    ros::Subscriber sub_depth = n.subscribe("/mobile_manip/d435i/depth/image_rect_raw", 1, depthCallback, ros::TransportHints().udp());
