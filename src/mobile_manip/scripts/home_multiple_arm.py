...
def main():
    number_of_robot = rospy.get_param("~number_of_robot")
    try:
        ...
        for robot_index in range(1, number_of_robot + 1):
          robot = InitializeGazeboRobot("robot{}".format(robot_index))
          success = robot.is_init_success
          if success:
              success &= robot.home_the_robot()
