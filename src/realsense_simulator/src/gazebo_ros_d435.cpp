#include "realsense_gazebo_plugin/gazebo_ros_d435.h"
...
namespace gazebo {
// Register the plugin
GZ_REGISTER_MODEL_PLUGIN(GazeboRosD435)

GazeboRosD435::GazeboRosD435() {}

GazeboRosD435::~GazeboRosD435() {
  ROS_DEBUG_STREAM_NAMED("d435_camera", "Unloaded");
}

void GazeboRosD435::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  ROS_INFO("D435 Gazebo ROS plugin loading.");

  D435Plugin::Load(_model, _sdf);

  // load parameters
  this->robot_namespace_ = "";
  if (_sdf->HasElement("robotNamespace"))
    this->robot_namespace_ =
      _sdf->GetElement("robotNamespace")->Get<std::string>() + "/d435i/";
...
void GazeboRosD435::OnNewFrame(const rendering::CameraPtr cam,
...
bool GazeboRosD435::FillPointCloudHelper(sensor_msgs::PointCloud2 &point_cloud_msg,
...
void GazeboRosD435::OnNewDepthFrame() {
  // get current time
  common::Time current_time = this->world->SimTime();

  D435Plugin::OnNewDepthFrame();
