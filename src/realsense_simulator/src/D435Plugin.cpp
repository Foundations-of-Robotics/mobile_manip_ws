...
#include "realsense_gazebo_plugin/D435Plugin.h"
...
/////////////////////////////////////////////////
void D435Plugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  // Output the name of the model
  std::cout
      << std::endl
      << "D435Plugin: The realsense_camera plugin is attached to model "
  ...
  do {
    ...
    if (name == "depthUpdateRate")
      ...
      throw std::runtime_error("Ivalid parameter for D435Plugin");
  ...
  if (!this->depthCam) {
    std::cerr << "D435Plugin: Depth Camera has not been found"
              << std::endl;
    return;
  }
  if (!this->ired1Cam) {
    std::cerr << "D435Plugin: InfraRed Camera 1 has not been found"
              << std::endl;
    return;
  }
  if (!this->ired2Cam) {
    std::cerr << "D435Plugin: InfraRed Camera 2 has not been found"
              << std::endl;
    return;
  }
  if (!this->colorCam) {
    std::cerr << "D435Plugin: Color Camera has not been found"
              << std::endl;
    return;
  }
  ...
  try {
    ...
    std::cerr << "D435Plugin: depthMap allocation failed: " << e.what()
              << std::endl;
    return;
  }
  ...
  this->newDepthFrameConn = this->depthCam->ConnectNewDepthFrame(
      std::bind(&D435Plugin::OnNewDepthFrame, this));

  this->newIred1FrameConn = this->ired1Cam->ConnectNewImageFrame(std::bind(
      &D435Plugin::OnNewFrame, this, this->ired1Cam, this->ired1Pub));

  this->newIred2FrameConn = this->ired2Cam->ConnectNewImageFrame(std::bind(
      &D435Plugin::OnNewFrame, this, this->ired2Cam, this->ired2Pub));

  this->newColorFrameConn = this->colorCam->ConnectNewImageFrame(std::bind(
      &D435Plugin::OnNewFrame, this, this->colorCam, this->colorPub));

  // Listen to the update event
  this->updateConnection = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&D435Plugin::OnUpdate, this));
}

/////////////////////////////////////////////////
void D435Plugin::OnNewFrame(const rendering::CameraPtr cam,
...
/////////////////////////////////////////////////
void D435Plugin::OnNewDepthFrame() {
